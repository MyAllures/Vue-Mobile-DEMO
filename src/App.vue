...
export default {
  ...
  computed: {
    ...
    ...mapState([
      'theme', 'isLoading', 'systemConfig', 'notificationVisible', 'notifications'
    ]),
    ...mapState('chatroom', {
    ...
    backOption () {
      let setting = this.pageSetting.backPage
      if (setting) {
        return {
          text: setting.text,
          cb: () => { this.$router.push({path: setting.path}) }
        }
